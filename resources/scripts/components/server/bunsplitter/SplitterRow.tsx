...
import { Button as BadgeButton } from '@/components/elements/button/index';
...
export default ({
    ...
    //console.log('from splitterrow', server.externalId);
    const getStats = () =>
    ...
    useEffect(() => {
        console.log('received serve robej', server);
    }, []);
        ...
        <>
            <StatusIndicatorBox className={className} $status={stats?.status}>
                <div css={tw`flex items-center col-span-12 sm:col-span-5 lg:col-span-6`}>
                    <div className={'hidden md:block icon mr-4'}>
                        <FontAwesomeIcon icon={faServer} />
                    </div>
                    <div css={tw``}>
                        <p css={tw`text-lg break-words mt-1`}>{server.name}</p>
                        <BadgeButton
                            size={BadgeButton.Sizes.XSmall}
                            className={
                                server.externalId.toString().includes('sub')
                                    ? '!text-gray-50 !bg-orange-600'
                                    : '!text-gray-50 !bg-blue-600'
                            }
                            disabled
                        >
                            {server.externalId.toString().includes('sub') ? 'Sub' : 'Primary'}
                        </BadgeButton>
                    </div>
                    <div>
                        <div css={tw`ml-2`}>
                            <Link to={`/server/${server.id}`}>
                                <Button isSecondary css={tw`mr-2`}>
                    ...
                    <div>
                        <div css={tw`ml-2`}>
                            <EditServerModal
                                externalid={server.externalId.toString()}
                                serverUuid={server.uuid}
                                serverName={server.name}
                                serverNestId={server.nestId}
                                serverEggId={server.eggId}
                                serverMemory={server.limits.memory}
                                serverDisk={server.limits.disk}
                                refreshServerList={refreshServerList}
                                refreshServerDetails={refreshServerDetails}
                            />
                        </div>
